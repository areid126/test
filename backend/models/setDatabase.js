...
async function getSet(id) {
    ...
    if (id && id.length === 24) {
        const sets = (await Set.find({ _id: mongoose.Types.ObjectId.createFromHexString(id) })).map(set => set.toObject());
...
async function deleteSet(id) {
    // Delete the Set
    if (id && id.length === 24) {
        await Set.findOneAndDelete({ _id: mongoose.Types.ObjectId.createFromHexString(id) });
...
async function updateSet(id, newSet) {
    ...
    if (id && id.length === 24 && newSet && newSet.title && newSet.cards && newSet.cards.length > 0) {
        ...
        const set = (await Set.findByIdAndUpdate(mongoose.Types.ObjectId.createFromHexString(id), update, {new: true}));
